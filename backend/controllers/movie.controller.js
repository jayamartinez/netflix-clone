...
export async function getTrendingMovie(req, res) {
    try {
        const data = await fetchTMDB("https://api.themoviedb.org/3/movie/popular?language=en-US&page=1")
...
export async function getSimilarMovies(req, res) {
    const { id } = req.params
    try {
        const data = await fetchTMDB(`https://api.themoviedb.org/3/movie/${id}/similar?language=en-US`)
